  ...
  class Terminology
    ...
    class ValueSet
      ...
      def process_with_expansions
        ...
        if @valueset_model&.expansion&.contains
          ...
          if valueset_toocostly || valueset_unclosed
            ...
            FHIRValidator.logger.debug("Processing expanded valueset: #{url}")
            process_expanded_valueset
          end
        else
          FHIRValidator.logger.debug("Processing composed valueset: #{url}")
          process_valueset
        end
      end
      ...
      def process_valueset
        FHIRValidator.logger.debug "Processing #{@valueset_model.url}"
        include_set = Set.new
      ...
      def filter_code_set(system, filter = nil, _version = nil)
        ...
        if CODE_SYS.include? system
          ...
          if SAB[system].nil?
            fhir_cs = FHIRValidator::Terminology::Codesystem
