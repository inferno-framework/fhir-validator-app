...
module FHIRValidator
  class Terminology
    ...
    def self.create_validators(type: :bloom, selected_module: :all, minimum_binding_strength: 'example', include_umls: true)
      ...
      validators = []
      umls_code_systems = Set.new(FHIRValidator::Terminology::ValueSet::SAB.keys)
      ...
      get_module_valuesets(selected_module, strengths).each do |k, vs|
        ...
        FHIRValidator.logger.debug "Processing #{k}"
    ...
    def self.get_module_valuesets(selected_module, strengths)
      if selected_module == :all
        ...
        module_vs_urls = FHIRValidator::Module.get(selected_module)
          .value_sets
        ...
        (module_vs_urls - module_valuesets.keys).each do |missing_vs_url|
          FHIRValidator.logger.error "Inferno doesn't know about valueset #{missing_vs_url} for module #{selected_module}"
        end
        module_valuesets
      end
    end
    ...
    # @return [FHIRValidator::Terminology::ValueSet] ValueSet
